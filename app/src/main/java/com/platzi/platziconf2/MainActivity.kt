...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        for(i in 0 until speakers.length()){
            //capturando un objeto json en una variable
            val speakerJsonObject = speakers.get(i) as JSONObject
            //instanciando nuestra clase model
            val speakerPojo = Speaker()
            speakerPojo.name = speakerJsonObject.getString("name")
            speakerPojo.jobtitle = speakerJsonObject.getString("jobtitle")
            speakerPojo.company = speakerJsonObject.getString("workplace")
            speakerPojo.biography = speakerJsonObject.getString("biography")
            speakerPojo.urlImage = speakerJsonObject.getString("image")
            speakerPojo.twitter = speakerJsonObject.getString("twitter")
            speakerPojo.category = speakerJsonObject.getInt("category")
            //creando una nueva coleccion y documento en firestore
            firebaseFirestore.collection("speakers").document().set(speakerPojo)

        }

        for(i in 0 until conferences.length()){
            val conferenceJsonObject = conferences.get(i) as JSONObject
            val conferencePOJO = Conference()
            conferencePOJO.description = conferenceJsonObject.getString("description")
            conferencePOJO.speaker = conferenceJsonObject.getString("speaker")
            conferencePOJO.tag = conferenceJsonObject.getString("tag")
            conferencePOJO.title = conferenceJsonObject.getString("title")
            //dandole formato a la fecha
            val cal = Calendar.getInstance()
            cal.timeInMillis = conferenceJsonObject.getLong("datetime") * 1000
            conferencePOJO.datetime = cal.time
            firebaseFirestore.collection("conferences").document().set(conferencePOJO)
        }
    }
